    ...
    <section>
      ...
      <header className="bg-white py-7 px-10 rounded-[20px] ">
          ...
          <div className='flex items-center  gap-[70px]'>
            <div className='w-full'>
              <DropDown options={dropDownOptions} defaultSelectedKeys={defaultDropDown} selectStyles='w-[200px]' />
            </div>
            <div className='w-full'>
              <DropDownWithLabel title='그룹' options={dropDownOptions} 
              defaultSelectedKeys={defaultDropDown} titleStyles={labelStyle} selectStyles='w-[235px]'/>
            </div>
            <div className='w-full'>
      ...
      <div className="bg-white py-10 px-10 rounded-[20px] mt-9">
        <header className="flex items-center justify-between">
          ...
          <DropDown options={dropDownOptions} defaultSelectedKeys={defaultDropDown}/>
            <HeaderDropDown
              options={viewOptions}
    ...
    {items.map((row) => (
      <TableRow key={row.productId} className="border-b-1">
        ...
        <TableCell className="truncate">
          <Link href="/admin/product/product-management/1" className="underline underline-offset-1">
            {row.productName}
          </Link>
        </TableCell>
        <TableCell className="truncate">{row.stockQuantity}</TableCell>
        <TableCell className="truncate">{row.salePrice}</TableCell>
        <TableCell className="truncate">{row.saleOrNot}</TableCell>
        <TableCell className="truncate">{row.purchaseLimit}</TableCell>
        <TableCell className="truncate">{row.modificationDate}</TableCell>
      </TableRow>
    ))}
  </TableBody>
</Table>
